...
CoordinatedGraphicsScene::CoordinatedGraphicsScene(CoordinatedGraphicsSceneClient* client, DamagePropagation damagePropagation)
    : m_client(client)
...
static TextureMapperLayer& texmapLayer(Nicosia::CompositionLayer& compositionLayer, DamagePropagation damagePropagation)
{
    auto& compositionState = compositionLayer.compositionState();
    if (!compositionState.layer) {
        compositionState.layer = makeUnique<TextureMapperLayer>(damagePropagation);
...
void CoordinatedGraphicsScene::updateSceneState()
{
    ...
    m_nicosia.scene->accessState(
        [this, &layersByBacking, &replacedProxiesToInvalidate](Nicosia::Scene::State& state)
        {
            ...
            {
                auto& rootLayer = texmapLayer(*state.rootLayer, m_damagePropagation);
            ...
            for (auto& compositionLayer : m_nicosia.state.layers) {
                auto& layer = texmapLayer(*compositionLayer, m_damagePropagation);
                compositionLayer->commitState(
                    ...
                    (const Nicosia::CompositionLayer::LayerState& layerState)
                    {
                        ...
                        if (layerState.delta.backdropFiltersChanged)
                            layer.setBackdropLayer(layerState.backdropLayer ? &texmapLayer(*layerState.backdropLayer, m_damagePropagation) : nullptr);
                        ...
                        if (layerState.delta.childrenChanged) {
                            layer.setChildren(WTF::map(layerState.children,
                                [this](auto& child) {
                                    return &texmapLayer(*child, m_damagePropagation);
                                }));
                        }

                        if (layerState.delta.maskChanged)
                            layer.setMaskLayer(layerState.mask ? &texmapLayer(*layerState.mask, m_damagePropagation) : nullptr);
                        if (layerState.delta.replicaChanged)
                            layer.setReplicaLayer(layerState.replica ? &texmapLayer(*layerState.replica, m_damagePropagation) : nullptr);
...
void CoordinatedGraphicsScene::ensureRootLayer()
{
    ...
    m_rootLayer = makeUnique<TextureMapperLayer>(m_damagePropagation);
