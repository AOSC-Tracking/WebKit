...
{
    if (!attributes.hasX() && (isAnimatedAttribute(SVGNames::xAttr) || hasAttribute(SVGNames::xAttr)))
        attributes.setX(x());

    if (!attributes.hasY() && (isAnimatedAttribute(SVGNames::yAttr) || hasAttribute(SVGNames::yAttr)))
        attributes.setY(y());

    if (!attributes.hasWidth() && (isAnimatedAttribute(SVGNames::widthAttr) || hasAttribute(SVGNames::widthAttr)))
        attributes.setWidth(width());

    if (!attributes.hasHeight() && (isAnimatedAttribute(SVGNames::heightAttr) || hasAttribute(SVGNames::heightAttr)))
        attributes.setHeight(height());

    if (!attributes.hasViewBox() && (isAnimatedAttribute(SVGNames::viewBoxAttr) || hasAttribute(SVGNames::viewBoxAttr)) && hasValidViewBox())
        attributes.setViewBox(viewBox());

    if (!attributes.hasPreserveAspectRatio() && (isAnimatedAttribute(SVGNames::preserveAspectRatioAttr) || hasAttribute(SVGNames::preserveAspectRatioAttr)))
        attributes.setPreserveAspectRatio(preserveAspectRatio());

    if (!attributes.hasPatternUnits() && (isAnimatedAttribute(SVGNames::patternUnitsAttr) || hasAttribute(SVGNames::patternUnitsAttr)))
        attributes.setPatternUnits(patternUnits());

    if (!attributes.hasPatternContentUnits() && (isAnimatedAttribute(SVGNames::patternContentUnitsAttr) || hasAttribute(SVGNames::patternContentUnitsAttr)))
        attributes.setPatternContentUnits(patternContentUnits());

    if (!attributes.hasPatternTransform() && (isAnimatedAttribute(SVGNames::patternTransformAttr) || hasAttribute(SVGNames::patternTransformAttr)))
