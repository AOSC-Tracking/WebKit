...
static void setGradientAttributes(SVGGradientElement& element, LinearGradientAttributes& attributes, bool isLinear = true)
{
    ...
    if (isLinear) {
        ...
        if (!attributes.hasX1() && (element.isAnimatedAttribute(SVGNames::x1Attr) || element.hasAttribute(SVGNames::x1Attr)))
            attributes.setX1(linear.x1());

        if (!attributes.hasY1() && (element.isAnimatedAttribute(SVGNames::y1Attr) || element.hasAttribute(SVGNames::y1Attr)))
            attributes.setY1(linear.y1());

        if (!attributes.hasX2() && (element.isAnimatedAttribute(SVGNames::x2Attr) || element.hasAttribute(SVGNames::x2Attr)))
            attributes.setX2(linear.x2());

        if (!attributes.hasY2() && (element.isAnimatedAttribute(SVGNames::y2Attr) || element.hasAttribute(SVGNames::y2Attr)))
