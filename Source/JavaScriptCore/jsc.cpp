    ...
    for (size_t i = 0; i < scripts.size(); i++) {
        ...
        case Script::CodeSource::File: {
            ...
            if (isModule) {
                ...
                JSValue resolvedString = identifierToJSValue(vm, resolvedIdentifier);
                RETURN_IF_EXCEPTION(scope, void());
                promise = globalObject->moduleLoader()->fetchModuleAndEvaluate(globalObject, resolvedString, jsUndefined(), jsUndefined());
                RETURN_IF_EXCEPTION(scope, void());
            } else {
